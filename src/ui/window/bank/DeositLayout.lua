...
local DeositLayout = class("DeositLayout",require("src.base.extend.CCLayerExtend"),IEventListener,function() 
	local container = display.newLayout(cc.size(970,385))
	container:setTouchEnabled(true)
	return container
end)

function DeositLayout:ctor(onlySaveStatus)
	self:super("ctor")
	self:addEvent(ST.COMMAND_PLAYER_GOLD_UPDATE)
	
	self:addChild(Coord.ingap(self,display.newText(display.trans("##2026"),30),"CC",0,"TT",-5))
	...
	self:addChild(Coord.ingap(self,walletbg,"CR",-60,"TT",-60))
	...
	self:addChild(Coord.ingap(self,banbg,"CL",90,"TT",-60))
	...
	self:addChild(Coord.outgap(walletbg,amountbg,"RR",50,"BT",-45))
	...
	if onlySaveStatus ~= 0 then
		--存款按钮
		local depositBtn = display.newButton("p_btn_1012.png","p_btn_1012.png")
		depositBtn:addChild(Coord.ingap(depositBtn, display.newImage("p_ui_1051.png"),"CC",-5,"CC",5))
		depositBtn:setPressedActionEnabled(true)
		self:addChild(Coord.ingap(self,depositBtn,"CR",-120,"BB",15))
		depositBtn:addTouchEventListener(function(t,e) 
			if e ~= ccui.TouchEventType.ended then return end
			...
			ConnectMgr.connect("normal.DepositConnect" , value,function(result) 
	...
	--取款按钮
	local withdrawBtn = display.newButton("p_btn_1012.png","p_btn_1012.png")
	withdrawBtn:addChild(Coord.ingap(withdrawBtn, display.newImage("p_ui_1052.png"),"CC",-5,"CC",5))
	withdrawBtn:setPressedActionEnabled(true)
	if onlySaveStatus == 0 then
		self:addChild(Coord.ingap(self,withdrawBtn,"CC",0,"BB",15))
	else
		self:addChild(Coord.ingap(self,withdrawBtn,"CL",120,"BB",15))
	end
	withdrawBtn:addTouchEventListener(function(t,e) 
		if e ~= ccui.TouchEventType.ended then return end
		local value = tonum(amount_num)
		if value <= 0 then
			return
		end
		if Player.bank < value then
			--存款不够
			display.showMsg(display.trans("##2033"))
			return
		end
		...
		ConnectMgr.connect("normal.WithdrawConnect" , value,psw,function(result) 
			if result ~= 0 then return end
			amountInput:setText("")
			amount_num = 0
...
--@override
function DeositLayout:handlerEvent(event,arg)
...
function DeositLayout:onCleanup()
	self:removeAllEvent()
end
