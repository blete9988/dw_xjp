...
local HaoChePiaoYiLostPanel = class("HaoChePiaoYiLostPanel",function() 
...
function HaoChePiaoYiLostPanel:ctor(sid,selfWinMoney,bankerName,bankerWinMoney,datas)
	SoundsManager.playSound("fcpy_lose")
	--播放烟雾
	local yanwu_image = display.newSprite()
	Coord.ingap(self,yanwu_image,"CC",0,"CC",0)
	yanwu_image:runAction(resource.getAnimateByKey("fcpy_yan"))
	yanwu_image:setScale(5)
	self:addChild(yanwu_image,0)
	self.bg_layout = nil
	...
	local function createBackground()
		--背景
		local bg_img = display.newImage("fcpy_result_lost_bg.png")
	...
	local function createSelf()
		...
		local self_result_image = display.newImage("fcpy_icon_47.png")
		self_result_image:setPosition(cc.p(251,150))
		self.bg_layout:addChild(self_result_image)

		local self_value_label = display.newText(math.abs(selfWinMoney),24,Color.GWJ_IIII)
	...
	local function createList()
		for i=1,#datas do
			...
			item:setPosition(373,410 - (i - 1)*42)
...
function HaoChePiaoYiLostPanel.show(sid,selfWinMoney,bankerName,bankerWinMoney,datas)
	if instance == nil then
		instance = HaoChePiaoYiLostPanel.new(sid,selfWinMoney,bankerName,bankerWinMoney,datas)
		display.getRunningScene():addChild(instance)
	end
	return instance
end

return HaoChePiaoYiLostPanel
