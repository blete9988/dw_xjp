...
local ClockCountDownItem = class("ClockCountDownItem",require("src.base.extend.CCLayerExtend"),function() 
	return display.newSprite("fqzs-fly-main-icon-horologe.png")
end)

function ClockCountDownItem:ctor(value,needRing,backfunction)
	self:super("ctor")
	local cdlabel = cc.Label:createWithCharMap(display.getTexture("game/feiqingzoushou/fqzs_number_1.png"),23,33,string.byte("0"))
	...
	self.m_soundhandler = SoundsManager.playSound("fqzs_countdown",true)
	...
	if delay >= 1000 then
		...
		if self.needRing and self.m_countdown <= 5 then
			...
			if self.m_countdown <= 5 and not self.isRing then
				...
				self.m_soundhandler = SoundsManager.playSound("fqzs_countdown_ring", true)
				self:runAction(resource.getAnimateByKey("fqzs_clock_ring_"..self.m_countdown,false,true))
				self.isRing = true
			end
		else
