...
local FistSuperManRulePanel = class("FistSuperManRulePanel",function() 
	local layout = display.extend("CCLayerExtend",display.newMask(cc.size(D_SIZE.width,D_SIZE.height)))
	return layout
end)

function FistSuperManRulePanel:ctor()
	local main_layout = display.newImage("#game/fistsuperman/fistSuperMan_rule_bg.png")
    ...
    local content_imgI = display.newDynamicImage("game/fistsuperman/fistSuperMan_rule_3.png")
    content_imgI:setAnchorPoint(cc.p(0,0))
    content_imgI:setPosition(cc.p(23,0))
    photo_scroll:addChild(content_imgI)

    local content_imgII = display.newDynamicImage("game/fistsuperman/fistSuperMan_rule_2.png")
    ...
    local content_imgIII = display.newDynamicImage("game/fistsuperman/fistSuperMan_rule_1.png")
...
function FistSuperManRulePanel.show()
	local ranle = FistSuperManRulePanel.new()
	display.getRunningScene():addChild(ranle)
end

return FistSuperManRulePanel
