...
local FistSuperManRewardPanel = class("FistSuperManRewardPanel",function() 
	local layout = display.extend("CCLayerExtend",display.newMask(cc.size(D_SIZE.width,D_SIZE.height),0))
	return layout
end)
local instance = nil
function FistSuperManRewardPanel:ctor(tipType,value,overfunction)
	self.overfunction = overfunction
	self:setTouchEnabled(true)
	if tipType == 1 then
		SoundsManager.playSound("FS_FreeSpinsResult")
		local effect_bg = display.newDynamicImage("game/fistsuperman/Free_Desc_WonCredits_bg.png")
		effect_bg:setPosition(cc.p(D_SIZE.width/2,D_SIZE.height/2))
		effect_bg:setScale(0.2)
		self:addChild(effect_bg)
		local effect_1 = display.newDynamicImage("game/fistsuperman/Free_Desc_WonCredits_effect2.png")
		local size = effect_1:getContentSize()
		effect_1:setPosition(cc.p(size.width/2,size.height/2))
		effect_bg:addChild(effect_1)
		local effect_2 = display.newDynamicImage("game/fistsuperman/Free_Desc_WonCredits_effect1.png")
		size = effect_2:getContentSize()
		effect_2:setPosition(cc.p(size.width/2,size.height/2))
		effect_bg:addChild(effect_2)
		local font_bg = display.newDynamicImage("game/fistsuperman/Free_Desc_WonCredits_cn.png")
		size = font_bg:getContentSize()
		font_bg:setPosition(cc.p(size.width/2,size.height/2))
		effect_bg:addChild(font_bg)
		local effect_label = ccui.TextAtlas:create(0,"game/fistsuperman/fontnumber_74X100_1.png",74,100,0)
	...
	elseif tipType == 2 then
		local layout_bg =  display.newDynamicImage("game/fistsuperman/fs_free_TSBG.png")
		layout_bg:setPosition(cc.p(D_SIZE.width/2,D_SIZE.height/2))
		layout_bg:setScale(0.2)
		self:addChild(layout_bg)
		local tit_label =  display.newDynamicImage("game/fistsuperman/fs_free_TS_1.png")
		Coord.ingap(layout_bg,tit_label,"CC",0,"CC",0)
		layout_bg:addChild(tit_label)
		local value_label =  ccui.TextAtlas:create(value,"game/fistsuperman/fontnumber_74X100_1.png",74,100,0)
	...
	elseif tipType == 3 then
		...
		layout_bg:setScale(0.2)
		self:addChild(layout_bg)
		local tit_label =  display.newDynamicImage("game/fistsuperman/fs_free_TS_2.png")
		Coord.ingap(layout_bg,tit_label,"CC",0,"CC",0)
		layout_bg:addChild(tit_label)
		local value_label =  ccui.TextAtlas:create(value,"game/fistsuperman/fontnumber_74X100_1.png",74,100,0)
...
function FistSuperManRewardPanel:setTimeOver(delay)
...
function FistSuperManRewardPanel:destroy()
	self:removeFromParent(true)
	instance = nil
end

function FistSuperManRewardPanel.show(tipType,value,overfunction)
	if instance == nil then
		instance = FistSuperManRewardPanel.new(tipType,value,overfunction)
		display.getRunningScene():addChild(instance,99)
	end
	return instance
end

return FistSuperManRewardPanel
