...
local FistSuperManStartButton = class("FistSuperManStartButton",function() return display.newButton(nil,nil,nil) end,require("src.base.extend.CCNodeExtend"),require("src.base.event.EventDispatch"))

function FistSuperManStartButton:ctor(type)
...
function FistSuperManStartButton:showType(type)
	self.type = type
	if self.type == 1 then
...
function FistSuperManStartButton:getType()
	return self.type
end

function FistSuperManStartButton:setText(value)
	self.number_txt:setString(value)
end

function FistSuperManStartButton:pauseShowType(type)
	self:showType(type)
...
return FistSuperManStartButton
