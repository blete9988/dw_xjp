...
local ClockCountDownItem = class("ClockCountDownItem",require("src.base.extend.CCLayerExtend"),function() 
	return display.newSprite("ui_brnn_1030.png")
end)

function ClockCountDownItem:ctor(betvalue)
	self:super("ctor")
	
	local cdlabel = cc.Label:createWithCharMap(display.getTexture("game/bairenniuniu/brnn_number_1.png"),23,33,string.byte("0"))
...
function ClockCountDownItem:stop()
	...
	self:setSpriteFrame("ui_brnn_1030.png")
...
function ClockCountDownItem:setTargetTimeStamp(value,needRing)
	...
	if needRing then
		self.m_soundhandler = SoundsManager.playSound("brnn_countdown", true)
	end
	...
	self:onTimer(function(t) 
		...
		if self.m_needRing and cd <= 5 then
			...
			if not self.isRing then
				...
				--播放闹铃剩余
				SoundsManager.stopAudio(self.m_soundhandler)
				self.m_soundhandler = SoundsManager.playSound("brnn_countdown_ring", true)
				
				self:runAction(resource.getAnimateByKey("brnn_clock_ring_" .. cd,false,true))
				self.isRing = true
			end
		else
