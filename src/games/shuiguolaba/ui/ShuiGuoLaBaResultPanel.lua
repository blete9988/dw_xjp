...
function FeiQingZouShouResultPanel:ctor(animalSid)
	local myself = 102498
	local self_iswin = false
	local zhuang_iswin = true
	local zhuang_number = 989812
	...
	local function showBg()
		...
		local self_name_label = display.newText("游客556692319",24,Color.GWJ_IIII)
		Coord.ingap(self_layout,self_name_label,"CC",0,"TT",-15)
		self_layout:addChild(self_name_label)
		if myself == 0 then  --未下注
			...
			local self_number_label = ccui.TextAtlas:create(myself,"game/feiqingzoushou/fqzs-number-24-34.png",24,34,0)
		...
		local zhuang_number_label = ccui.TextAtlas:create(zhuang_number,"game/feiqingzoushou/fqzs-number-24-34.png",24,34,0)
	    Coord.ingap(zhuangjia_layout,zhuang_number_label,"CC",0,"CC",-50)
	    zhuangjia_layout:addChild(zhuang_number_label)

		for i=1,5 do
			if i < 5 then
				local item = createItem(i,"我爱天天牛牛",999000009)
				item:setPosition(100,220 - (i - 1)*36.5)
				self.bg_layout:addChild(item)
			else
				local item = createItem(i,"我爱天天牛牛的老婆",999000009)
				item:setPosition(100,220 - (i - 1)*36.5)
				self.bg_layout:addChild(item)
			end
		end
...
function FeiQingZouShouResultPanel.show(animalSid)
	if instance == nil then
		instance = FeiQingZouShouResultPanel.new(animalSid)
