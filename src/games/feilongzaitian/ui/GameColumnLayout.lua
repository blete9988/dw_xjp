...
--项总数
local ITEM_COUNT=0
local FeiLongZaiTianData =  require("src.games.feilongzaitian.data.FeiLongZaiTian_element_data")
local base_height = 0
local base_width = 0
local bottomPy = 0
local topPy = 0
...
local bottomPy = 0
local topPy = 0
local soundController = require("src.games.feilongzaitian.data.FeiLongZaiTianSoundController").getInstance()
-- maxcount元素总类数量
function GameColumnLayout:ctor(maxcount,index)
	...
	--切割图片
	local holeStencil = cc.Sprite:create("game/feilongzaitian/flzt_1.png")
	...
	for i=1,item_length do
		local data = FeiLongZaiTianData.new(self.sequence[i])
		local item = require("src.games.feilongzaitian.ui.GameImage").new(data)
		item.index = index
...
function GameColumnLayout:setSpecial()
	SoundsManager.playSound("flzt_maybeBlow")
	self:timeOver()
	self.isSpecial = true
	self.specialTime = 0
