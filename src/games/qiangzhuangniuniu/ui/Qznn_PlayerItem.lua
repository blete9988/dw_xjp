...
local Qznn_PlayerItem = class("Qznn_PlayerItem",require("src.base.extend.CCLayerExtend"),function() 
	return display.newLayout(cc.size(130,190))
end)

function Qznn_PlayerItem:ctor(index)
	self:super("ctor")
	self.index = index
...
--设置准备图片坐标
function Qznn_PlayerItem:setReadyPicPos(pos)
	self.m_readyPic:setPosition(pos)
end
--设置加倍图标坐标
function Qznn_PlayerItem:setTimesPicPos(pos)
	self.m_timesPic:setPosition(pos)
end
--开始倒计时
function Qznn_PlayerItem:beganCountdown(tm)
	if tm == 0 then
...
--更新信息
function Qznn_PlayerItem:updateInfo()
	self:setPlayerInfo(self.playerInfo)
end
function Qznn_PlayerItem:updateGold()
	if not self.playerInfo then return end
	self.m_goldTxt:setString(string.cnspNmbformat(self.playerInfo.gold))
end
--更新加倍显示图标
function Qznn_PlayerItem:updateTimesStatus()
	if not self.playerInfo then return end
...
--更新准备状态图标
function Qznn_PlayerItem:updateReadyStatus()
	if not self.playerInfo then return end
...
--显示庄家图标
function Qznn_PlayerItem:showMasterIcon(noPlayAnim)
...
--显示输赢结果
function Qznn_PlayerItem:showResult()
	if not self.playerInfo then return end
	...
	if value > 0 then
		...
		local anim = display.newParticle("game/qiangzhuangniuniu/particle/star01.plist")
...
--设置玩家信息
function Qznn_PlayerItem:setPlayerInfo(playerInfo)
