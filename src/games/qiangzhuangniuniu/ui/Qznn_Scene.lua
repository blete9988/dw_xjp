...
local Qznn_Scene = class("Qznn_Scene",require("src.base.extend.CCSceneExtend"),IEventListener)
function Qznn_Scene:ctor(room)
	self:super("ctor")
	self.qznn_gameMgr = require("src.games.qiangzhuangniuniu.data.Qznn_GameMgr").getInstance()
	self.qznn_gameMgr:setRoom(room)
	self.qznn_gameMgr:setQznnSelf(self)
	...
	self:addEvent(ST.COMMAND_GAMEQZNN_ENTRYROOM)
	self:addEvent(ST.COOMAND_GAMEQZNN_BEGAN)
	self:addEvent(ST.COOMAND_GAMEQZNN_SEND_OVER)
	self:addEvent(ST.COMMAND_GAMEQZNN_RESULT)
	self:addEvent(ST.COMMAND_GAMEQZNN_RESULT_OVER)
	self:addEvent(ST.COMMAND_GAMEQZNN_SHOWDOWN)
	self:addEvent(ST.COMMAND_GAMEQZNN_PLAYERCHANGE)
	self:addEvent(ST.COMMAND_GAMEQZNN_READYSTATUS_UPDATE)
	self:addEvent(ST.COMMAND_GAMEQZNN_BEGAN_GET_MASTER)
	self:addEvent(ST.COMMAND_GAMEQZNN_MASTER_VALUE)
	self:addEvent(ST.COMMAND_GAMEQZNN_BEGAN_ADD_TIMES)
	self:addEvent(ST.COMMAND_GAMEQZNN_TIMES_VALUE)
	
	self:addChild(Coord.ingap(self,display.newImage("#game/qiangzhuangniuniu/qznn_desktop_background.jpg"),"CC",0,"CC",0))
	--桌面层
	self.m_desktopLayout = require("src.games.qiangzhuangniuniu.ui.DesktopLayout").new()
	...
	ruleBtn:addTouchEventListener(function(t,e) 
		...
		display.showWindow("src.games.qiangzhuangniuniu.ui.Qznn_RuleWindows")
	end)
	...
	self.qznn_gameMgr:initgame()
end
--初始化操作按钮
function Qznn_Scene:m_initOperateButton()
	--换桌按钮
	...
	autoBtn.status = ST.TYPE_GAMEQZNN_NOT_AUTO
	...
	changeBtn:addTouchEventListener(function(t,e) 
		if e ~= ccui.TouchEventType.ended then return end
		ConnectMgr.connect("src.games.qiangzhuangniuniu.connect.Qznn_ChangeRoomConnect")
	end)
	--自动执行按钮点击
	autoBtn:addTouchEventListener(function(t,e) 
		if e ~= ccui.TouchEventType.ended then return end
		local myinfo = self.qznn_gameMgr:getMineInfo()
		if autoBtn.status == ST.TYPE_GAMEQZNN_NOT_AUTO then
			autoBtn.status = ST.TYPE_GAMEQZNN_AUTO
			--自动执行
	...
	operateBtn:addTouchEventListener(function(t,e) 
		if e ~= ccui.TouchEventType.ended then return end
		local myinfo = self.qznn_gameMgr:getMineInfo()
		if self.qznn_gameMgr:isPlaying() then
			--已经开始
			...
			if myinfo and myinfo:isReady() then
				--已准备,请求取消准备
				ConnectMgr.connect("src.games.qiangzhuangniuniu.connect.Qznn_ReadyStatusConnect",3,function(result)
...
--初始化顶部面板
function Qznn_Scene:m_initTopView()
	...
	baseBetBg:addChild(Coord.ingap(baseBetBg,display.newText(display.trans("##7001",self.qznn_gameMgr.room.minBets),26,Color.GREY),"CC",0,"CC",0))
	...
	--玩家金币文本
	local goldTxt = cc.Label:createWithCharMap("game/qiangzhuangniuniu/qznn_number_1.png",24,34,string.byte("0"))
...
--初始化底部面板
function Qznn_Scene:m_initBottomView()
...
--更新按钮状态
function Qznn_Scene:updateButtonStatus()
	self.m_autoBtn:setVisible(true)
	if self.m_autoBtn.status == ST.TYPE_GAMEQZNN_AUTO then
	...
	local myinfo = self.qznn_gameMgr:getMineInfo()
	if self.qznn_gameMgr.gamestatus == ST.TYPE_GAMEQZNN_PLAYING then
		--已经开始,隐藏换桌按钮
		self.m_changeBtn:setVisible(false)
		if self.qznn_gameMgr.playStatus == ST.TYPE_GAMEQZNN_PLAYSTATUS_PLAYING then
...
--请求准备
function Qznn_Scene:connectRequestReady()
	if self.qznn_gameMgr:getMineInfo():isReady() then return end
	ConnectMgr.connect("src.games.qiangzhuangniuniu.connect.Qznn_ReadyStatusConnect",2,function(result)
		if result ~= 0 then return end
		self.m_changeBtn:setVisible(false)
		self.m_operateBtn:setTitleText(display.trans("##7006"))
...
--请求摊牌
function Qznn_Scene:connectRequestShowdown()	
	if self.qznn_gameMgr:getMineInfo():isShowdown() then return end
	ConnectMgr.connect("src.games.qiangzhuangniuniu.connect.Qznn_ShowdownConnect",function(result)
		if result ~= 0 then return end
		self.m_operateBtn:setVisible(false)
		--self.m_desktopLayout:showdown(self.qznn_gameMgr:getMineInfo())
	end)
end
--自动执行
function Qznn_Scene:onAutoExcute()
	if self.m_autoBtn.status ~= ST.TYPE_GAMEQZNN_AUTO then return end
	
	if self.qznn_gameMgr:isPlaying() then
		if not self.qznn_gameMgr:getMineInfo():isShowdown() then
			--请求摊牌
			SoundsManager.playSound("main_sound_click")
			self:connectRequestShowdown()
		end
	elseif not self.qznn_gameMgr:getMineInfo():isReady() then
		--请求准备
		SoundsManager.playSound("main_sound_click")
		self:connectRequestReady()
	end
...
--@override
function Qznn_Scene:handlerEvent(event,arg)	
	if event == ST.COMMAND_PLAYER_GOLD_UPDATE then
		--金币更新
		self.m_goldTxt:setString(Player.gold)
	elseif event == ST.COMMAND_GAMEQZNN_ENTRYROOM then
		--进入房间
		self.m_desktopLayout:init()
		
		self.m_quitebtn:disabledExitButton(self.qznn_gameMgr.gamestatus == ST.TYPE_GAMEQZNN_PLAYING)
		
		--重置自动按钮的状态
		self.m_autoBtn:setTitleText(display.trans("##7007"))
		self.m_autoBtn.status = ST.TYPE_GAMEQZNN_NOT_AUTO
		self:updateButtonStatus()
		
		self.m_goldTxt:setString(Player.gold,true)
		self.m_historyTxt:setString(0,true)
		self.m_previousTxt:setString(0,true)
	elseif event == ST.COMMAND_GAMEQZNN_BEGAN_GET_MASTER then
		--开始抢庄
		
		--移除准备状态改变监听
		self:removeEvent(ST.COMMAND_GAMEQZNN_READYSTATUS_UPDATE)
	...
	elseif event == ST.COMMAND_GAMEQZNN_MASTER_VALUE or event == ST.COMMAND_GAMEQZNN_TIMES_VALUE then
		--抢庄倍数 和 加倍倍数 接收
		self.m_desktopLayout:updatePlayersTimesStatus()
	elseif event == ST.COMMAND_GAMEQZNN_BEGAN_ADD_TIMES then
		--开始加倍
		self.m_desktopLayout:showTimesOperateBtn()
		self.m_desktopLayout:updatePlayersTimesStatus(true)
	elseif event == ST.COOMAND_GAMEQZNN_BEGAN then
		--开始发牌
		SoundsManager.playSound("qznn_start")
		--执行发牌
		self.m_desktopLayout:beganDeal()
	elseif event == ST.COOMAND_GAMEQZNN_SEND_OVER then
		--发牌结束
		self:updateButtonStatus()
		self:onAutoExcute()
	elseif event == ST.COMMAND_GAMEQZNN_SHOWDOWN then
		--摊牌
	...
	elseif event == ST.COMMAND_GAMEQZNN_RESULT then
		--收到比牌结果
		SoundsManager.playSound("qznn_kaipai")
		self.m_quitebtn:disabledExitButton(false)
		self.m_desktopLayout:showResultEffect()
		
	elseif event == ST.COMMAND_GAMEQZNN_RESULT_OVER then
		--结果显示结束
		self.m_historyTxt:setString(self.qznn_gameMgr.allRecord)
		self.m_previousTxt:setString(self.qznn_gameMgr.curentRecord,true)
		self:updateButtonStatus()
		self:onAutoExcute()
		self:addEvent(ST.COMMAND_GAMEQZNN_READYSTATUS_UPDATE)
		CommandCenter:sendEvent(ST.COMMAND_PLAYER_GOLD_UPDATE)
	elseif event == ST.COMMAND_GAMEQZNN_READYSTATUS_UPDATE then
		--玩家准备状态发生改变
		self.m_desktopLayout:updatePlayersReadyStatus()
	elseif event == ST.COMMAND_GAMEQZNN_PLAYERCHANGE then
		--玩家变动
		if arg.type then
			--加入房间
	...
	elseif event == ST.COMMAND_MAINSOCKET_BREAK then
		...
		local room = require("src.games.qiangzhuangniuniu.data.Qznn_GameMgr").getInstance().room
		display.enterScene("src.ui.ReloginScene",{room})
	end
end

function Qznn_Scene:Quit()
	ConnectMgr.connect("gamehall.QuitRoomConnect")
	display.enterScene("src.ui.scene.MainScene")
end

--@override
function Qznn_Scene:onCleanup()
	mlog("关闭面板。。。。。")
	self:removeAllEvent()
