...
function DesktopLayout:ctor()
	...
	local pokerGroups = {}
	for i = 1,5 do
		item = require("src.games.qiangzhuangniuniu.ui.PokerGroupItem").new()
		if i > 1 then
			item:setNotself()
		end
		...
		pokerGroups[i] = item
		--第一个为玩家自己
		item = require("src.games.qiangzhuangniuniu.ui.Qznn_PlayerItem").new(i)
...
function DesktopLayout:init()
	...
	self:initPlayers()
	if gameMgr.gamestatus == ST.TYPE_GAMEQZNN_PLAYING then
		if gameMgr.playStatus == ST.TYPE_GAMEQZNN_PLAYSTATUS_MASTER then
			--正在抢庄
			self:showTimesOperateBtn(true)
		elseif  gameMgr.playStatus == ST.TYPE_GAMEQZNN_PLAYSTATUS_ADDTIMES then
			--正在加倍
...
function DesktopLayout:initPlayers()
	...
	for i = 1,#self.m_playerItems do
		...
		index = (index + 1)%ST.TYPE_GAMEQZNN_MAX_PLAYERS
	end
end
...
function DesktopLayout:beganDeal()
	...
	self:runAction(cc.Sequence:create({
		...
		cc.CallFunc:create(function(t) 
			...
			CommandCenter:sendEvent(ST.COOMAND_GAMEQZNN_SEND_OVER)
		end),
	}))
end
...
function DesktopLayout:m_sendCard(pokerIndex)
	local index = require("src.games.qiangzhuangniuniu.data.Qznn_GameMgr").getInstance():getMaster().clientIndex
	local len = 0
	local flycard
	for i = 1,ST.TYPE_GAMEQZNN_MAX_PLAYERS do
		...
		index = (index%ST.TYPE_GAMEQZNN_MAX_PLAYERS) + 1
	end
end
--移除加倍操作弹层
...
function DesktopLayout:showTimesOperateBtn(ismaster)
	...
	--倍率配置
	local config
	local mineInfo = require("src.games.qiangzhuangniuniu.data.Qznn_GameMgr").getInstance():getMineInfo()
	...
	local targetTimeStamp = require("src.games.qiangzhuangniuniu.data.Qznn_GameMgr").getInstance().cdStamp
	...
	local function btncallback(t,e)
		...
		local path
		if ismaster then
			path = "src.games.qiangzhuangniuniu.connect.Qznn_GetMasterConnect"
		else
			path = "src.games.qiangzhuangniuniu.connect.Qznn_AddTimesConnect"
		end
...
function DesktopLayout:resetDesktop()
	require("src.games.qiangzhuangniuniu.data.Qznn_GameMgr").getInstance():resetPlayers()
	
	self:resetPoker()
	for i = 1,#self.m_playerItems do
		self.m_playerItems[i]:updateInfo()
	end
	CommandCenter:sendEvent(ST.COMMAND_GAMEQZNN_RESULT_OVER)
end
--显示结果动画
function DesktopLayout:showResultEffect()
	local players = require("src.games.qiangzhuangniuniu.data.Qznn_GameMgr").getInstance():getPlayers()
