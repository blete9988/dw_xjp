...
function BetButton:ctor(betvalue)
	...
	self:loadTextureNormal(string.format("jsys_bet_%s.png",betvalue),1)
	self:loadTexturePressed(string.format("jsys_bet_%s_select.png",betvalue),1)
	self:loadTextureDisabled(string.format("jsys_bet_%s_disable.png",betvalue),1)
	self:setScale(1.4)
	self.disable = false
