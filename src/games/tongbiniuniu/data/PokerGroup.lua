...
function PokerGroup:getTypePic()
	if self.groupTypeValue then
		return string.format("ui_tbnn_result_%s%s.png",self.groupType,self.groupTypeValue)
	else
		return string.format("ui_tbnn_result_%s.png",self.groupType)
	end
end
function PokerGroup:getTypeSound()
	if self.groupType == 3 then
		return "tbnn_pokertype_3"
	elseif self.groupType ~= 4 then
		return "tbnn_pokertype_5"
	else
		return "tbnn_pokertype_4" .. self.groupTypeValue
	end
end
--牌是否大于等于牛牛大
...
function PokerGroup:bytesRead(data)
	...
	for i = 1,5 do
		...
		self.pokers[i] = require("src.games.tongbiniuniu.data.PokerData").new(sid)
	end
	--组合的值
