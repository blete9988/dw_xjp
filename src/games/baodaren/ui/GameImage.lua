...
function GameImage:ctor(data)
	...
	self.controller:addEventListener("BDR_CONTROLLER_FREE_STATE",function(event,eventtype,parms)
		self.isFree = true
		self:updateIcon()
	end)
	self.controller:addEventListener("BDR_CONTROLLER_NORMAL_STATE",function(event,eventtype,parms)
		self.isFree = false
		self:updateIcon()
	end)
end
