...
--项总数
local ITEM_COUNT=0
local BaoDaRenData =  require("src.games.baodaren.data.BaoDaRen_element_data")
local base_height = 0
local base_width = 0
local bottomPy = 0
local topPy = 0
...
local bottomPy = 0
local topPy = 0
local soundController = require("src.games.baodaren.data.BaoDaRenSoundController").getInstance()
-- maxcount元素总类数量
function GameColumnLayout:ctor(maxcount,index)
	...
	--切割图片
	local holeStencil = cc.Sprite:create("game/baodaren/bdr_1.png")
	...
	for i=1,item_length do
		local data = BaoDaRenData.new(self.sequence[i])
		local item = require("src.games.baodaren.ui.GameImage").new(data)
		item.index = index
...
function GameColumnLayout:setSpecial(indexs)
	SoundsManager.playSound("bdr_maybeBlow")
	self:timeOver()
	self.isSpecial = true
	self.specialTime = 0
...
function GameColumnLayout:turn()
	...
	if self.stop then
		...
		for i=1,length do
			local item = self.items[i]
			local py = item.index * base_height + base_height * 0.5
			item:setPositionY(py - 10)
			item:runAction(cc.Sequence:create({cc.MoveTo:create(0.1,cc.p(item:getPositionX(),py)),
...
function GameColumnLayout:hideNotShowItem()
	self.items[1]:setVisible(false)
	self.items[6]:setVisible(false)
end
