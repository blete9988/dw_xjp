...
local BaoDaRenStartButton = class("BaoDaRenStartButton",function() return display.newButton(nil,nil,nil) end,require("src.base.extend.CCNodeExtend"),require("src.base.event.EventDispatch"))

function BaoDaRenStartButton:ctor(type)
...
function BaoDaRenStartButton:showType(type)
	self.type = type
	if self.type == 1 then
...
function BaoDaRenStartButton:getType()
	return self.type
end

function BaoDaRenStartButton:setText(value)
	self.number_txt:setString(value)
end

function BaoDaRenStartButton:pauseShowType(type)
	self:showType(type)
