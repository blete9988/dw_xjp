...
local BaoDaRenScene = class("BaoDaRenScene",function()
...
function BaoDaRenScene:ctor(room)
	self:super("ctor")
	self.room = room
	...
	self.soundController = require("src.games.baodaren.data.BaoDaRenSoundController").getInstance()
	self:initUi()
	self:initData()
...
--初始化UI
function BaoDaRenScene:initUi()
	local main_layout = require("src.games.baodaren.ui.BaoDaRenSceneUiPanel").new()
	...
	for i=1,COLUMN_COUNT do
		local gameLayout=require("src.games.baodaren.ui.GameColumnLayout").new(ELEMENT_MAXCOUNT,i)
	...
	self.main_layout:addEventListener("UI_MAIN_EVENT",function(event,eventtype,parms)
		...
		elseif eventtype == "FREE_OVER_EVENT" then	--免费结束
			self:desktopClean()
			self:hideGameLayoutNotShow()
			if parms > 40000000 then
				self:shootGold()
			end
		end
	end)
	local maskLayout = display.newLayout(self.content_layout:getContentSize())
	maskLayout:setBackGroundColorType(ccui.LayoutBackGroundColorType.solid)
	maskLayout:setBackGroundColor(Color.GREY)
	maskLayout:setBackGroundColorOpacity(100)
	maskLayout:setVisible(false)
	self.content_layout:addChild(maskLayout,1)
	self.maskLayout = maskLayout
	self:hideGameLayoutNotShow()
end
--初始化数据
function BaoDaRenScene:initData()
	...
	ConnectMgr.connect("src.games.baodaren.content.BaoDaRenInitConnect",function(result)
...
--启动旋转
function BaoDaRenScene:runTurn()
	local i=0
...
--有转动停止
function BaoDaRenScene:gameColumnStop(index)
	...
	if not self.isAtOnceStop and self.game_layouts[next_column].isManual then
		if next_column >= 3 and self.content_data:getHasBonusByCloum(next_column) >= 2 then
			self.game_layouts[next_column]:setSpecial()
		else
...
--播放单个特殊元素 index:列 
function BaoDaRenScene:playSingleSpecial(column)
	local index = self.game_layouts[column]:getIndexByType(7)
	if index>0 then
		SoundsManager.playSound("bdr_free")
	end
end
--播放连线元素的边框
function BaoDaRenScene:playRect(line_data)
	self:hideRect()
	local locations = line_data.locations
	local count = line_data.count
	for i=1,count do
		local effect = self:createEffect(i,locations[i])
		effect:setLocalZOrder(3)
		self.game_layouts[i]:hideByIndex(locations[i])
		if effect then
			effect:setRect()
			if i == 1 then
...
--播放普通线
function BaoDaRenScene:playNormalLine(linedatas)
	...
	if self.main_layout.isFreeState then
		SoundsManager.playSound("bdr_Scoring02",true)
	else
		SoundsManager.playSound("bdr_Scoring01",true)
	end
	local free_soundName = self.content_data:getFreeItemSoundName()
	if free_soundName then
		SoundsManager.playSound(free_soundName)
	end
end
--播放轮播线条
function BaoDaRenScene:playBlinkLine(linedatas)
	local length = #linedatas
	local index = 0
	if length == 0 then
		return
	end
	self.maskLayout:setVisible(true)
	local topItem = nil
...
--播放吃剑
function BaoDaRenScene:playChiJian(multipleValue)
	SoundsManager.playSound("bdr_sfbj")
	local mask_layout = display.newLayout(cc.size(D_SIZE.width,D_SIZE.height))
	mask_layout:setTouchEnabled(true)
	local chijian_sprite = display.newSprite()
	chijian_sprite:setScale(3)
	chijian_sprite:runAction(cc.Sequence:create({
		resource.getAnimateByKey("bdr_action_cj"),
		cc.CallFunc:create(function(sender)
			mask_layout:removeFromParent(true)
		end)
	}))
	Coord.ingap(mask_layout,chijian_sprite,"CC",0,"CC",0)
	mask_layout:addChild(chijian_sprite)
	--倍数
	local multiple_image = display.newImage("baodaren_multipliers_"..multipleValue..".png")
	local mask_size = mask_layout:getContentSize()
	multiple_image:setVisible(false)
	multiple_image:setPosition(cc.p(mask_size.width/2,mask_size.height/2))
	mask_layout:addChild(multiple_image)
	multiple_image:setScale(5)
	multiple_image:runAction(cc.Sequence:create({
		cc.DelayTime:create(2.5),
		cc.CallFunc:create(function(sender)
			sender:setVisible(true)
			self.main_layout:updateFreeMultiplier(multipleValue,true)
		end),
		cc.ScaleTo:create(0.2,1)
	}))
	self.main_layout:addChild(mask_layout,4)
end
--播放免费奖特效
function BaoDaRenScene:playBonusEffect(bonusCloums)
	-- self:showBonusMask()
	...
	for i=1,#bonusCloums do
		...
		effect:playBonu(function()
			...
			if count == #bonusCloums then
				...
				-- SoundsManager.stopAudio("bdr_bonus")
...
--创建线条
function BaoDaRenScene:getLineItem(lineSid,zorder)
	...
	local line_img = display.newDynamicImage("game/baodaren/line/FS_Line_"..lineSid..".png")
	line_img:setAnchorPoint(cc.p(0,0))
	line_img:setPosition(10,0)
	self.content_layout:addChild(line_img,2)
	self.lineItems[lineSid] = line_img
	return line_img
end
--移除线条
function BaoDaRenScene:removeLines()
	for i=1,#self.lineItems do
		self.lineItems[i]:removeFromParent()
	end
	self.lineItems={}
end
--隐藏所有边框
function BaoDaRenScene:hideRect()
	for i=1,#self.effectList do
		local columns = self.effectList[i]
		for n=1,#columns do
			if columns[n] then
				self.effectList[i][n]:hideRect()
				self.effectList[i][n]:setLocalZOrder(0)
			end
		end
	end
end
--加载中奖元素的控件
function BaoDaRenScene:loadEffects(rewardDatas)
	for i=1,#rewardDatas do
		local data = rewardDatas[i]
		local line_data = require("src.games.baodaren.data.BaoDaRen_line_data").new(data.lineType)
...
--根据索引获取特效元素
function BaoDaRenScene:createEffect(column,line)
	...
	if not effect then
		...
		local data = require("src.games.baodaren.data.BaoDaRen_element_data").new(etype)
		effect = require("src.games.baodaren.ui.GameEffect").new(data,self.main_layout.isFreeState)
...
function BaoDaRenScene:playGame()
	if self.playgame_action then return end
	SoundsManager:stopAudio("bdr_addScoring")
	ConnectMgr.connect("src.games.baodaren.content.BaoDaRenPlayConnect",self.controller:getBetMoney(),function(result)
		if result then
			self:desktopClean()
			...
			self.column_count = 0
			if not self.main_layout.isFreeState then
				self.turn_name = "bdr_turn_"..math.random(1,10)
				SoundsManager.playSound(self.turn_name)
			end
...
--[[本次的旋转结束]]
function BaoDaRenScene:gameOver()
	self:hideGameLayoutNotShow()
	self.soundController:roundOver()
	if self.turn_name then
		SoundsManager.stopAudio(self.turn_name)
		self.turn_name = nil
	end
	local delay = 0.2
	...
	if self.content_data.isHisChijian then
		local callback = cc.CallFunc:create(function()
			self:playChiJian(self.content_data.freeMultiple)
		end)
		table.insert(sequence_array,callback)
		table.insert(sequence_array,cc.DelayTime:create(3.5))
	end
...
--[[本回合结束]]
function BaoDaRenScene:stopGame()
...
--清场
function BaoDaRenScene:desktopClean()
...
function BaoDaRenScene:hideGameLayoutNotShow()
	for i=1,#self.game_layouts do
		self.game_layouts[i]:hideNotShowItem()
	end
end
--[[自动开始]]
function BaoDaRenScene:autoPlayGame(delay)
...
--移除特效控件
function BaoDaRenScene:removeEffect()
	for i=1,#self.effectList do
...
--发射炮弹
function BaoDaRenScene:shootGold()
	SoundsManager.playSound("bdr_goldBlow")
	...
	local function singleGold()
		local goldSprite=require("src.games.baodaren.ui.GoldSprite").new()
